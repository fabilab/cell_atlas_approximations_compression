...
if __name__ == "__main__":
    ...
    parser.add_argument('--n-macro', type=int, help="Number of macrogenes", required=True)
    parser.add_argument('--n-hvg', type=int, help="Number of highly variable  genes", required=True)
    parser.add_argument('--epochs', type=int, help="Number of epochs in metric learning", required=True)
    parser.add_argument('--pretrain-epochs', type=int, help="Number of epochs in pretraining", required=True)
    parser.add_argument('--kind', choices=["pretrain", "final", "25", "50"], default="final", type=str, help="Whether to look at pretrained, intermediate, or final trained model output")
    parser.add_argument('--kingdom', choices=["animal", "plant", "all"], default="all", type=str, help="Whether to look at animals, plants, or all species")
    parser.add_argument('--umap-dim', choices=[2, 3], default=2, type=int, help="Whether to plot UMAP in 2D or 3D")
    args = parser.parse_args()

    output_fdn = pathlib.Path(f"/mnt/data/projects/cell_atlas_approximations/reference_atlases/data/saturn/output_nmacro{args.n_macro}_nhvg{args.n_hvg}_epochs_p{args.pretrain_epochs}_m{args.epochs}")
    ...
    if args.kind == "pretrain":
        saturn_h5ad = output_fdn / "saturn_results" / "adata_pretrain.h5ad"  # 25 epochs for now, fix this
    elif args.kind != "final":
        saturn_h5ad = output_fdn / "saturn_results" / f"adata_ep_{args.kind}.h5ad"  # 25 epochs for now, fix this
    else:
        saturn_h5ad = output_fdn / "saturn_results" / "final_adata.h5ad"  # 25 epochs for now, fix this
    ...
    for species, datum in df.iterrows():
        ...
        if str(h5ad_fn).startswith("/srv/scratch"):
            h5ad_fn = pathlib.Path(f"/mnt/data/projects/cell_atlas_approximations/reference_atlases/data/saturn/h5ads/{species}.h5ad")
        if not h5ad_fn.exists():
            species_orig = species.split("-")[0]
            h5ad_fn = pathlib.Path(f"/mnt/data/projects/cell_atlas_approximations/reference_atlases/data/saturn/h5ads/{species_orig}.h5ad")
        if not h5ad_fn.exists():
    ...
    if args.umap_dim == 2:
        ...
        for i, ct in enumerate(cell_types):
            print(i+1, ct)
            sc.pl.umap(adata, color="cell_type", title="Cell Type", groups=[ct], add_outline=True, size=18, palette=palette)
            fig4 = plt.gcf()
            fig4.set_size_inches(8, 5)
            fig4.axes[0].legend(ncol=1, fontsize=6, bbox_to_anchor=(1,1), bbox_transform=fig4.axes[0].transAxes)
    ...
    if args.kingdom != "plant":
        ...
        while True:
            ...
            ct_knn = adata_h.obs['cell_type'].iloc[np.asarray(knni.ravel().to('cpu'))].values.reshape(knni.shape)
        ...
        if False:
            ...
            skpl = ConfusionMatrixDisplay.from_predictions(
                *df.loc[df['species'] == species, ['cell_type', 'closest_human']].values.T,
        ...
        conf_matrix = confusion_matrix(df['cell_type'], df['closest_human'], labels=labels_unsorted)
        conf_matrix_norm = 1.0 * (conf_matrix.T / (0.001 + conf_matrix.sum(axis=1))).T

        if False:
            link = linkage(1 - conf_matrix_norm, method='average', optimal_ordering=True)
            idx_labels_sorted = leaves_list(link)
            conf_matrix_sorted = conf_matrix_norm[idx_labels_sorted][:, idx_labels_sorted]
        ...
        frac_exact = 1.0 * conf_matrix_with_human[np.arange(len(conf_matrix_with_human)), np.arange(len(conf_matrix_with_human))].sum() / conf_matrix_with_human.sum()
        tmp = conf_matrix_with_human.copy()
        tmp = 1.0 * (tmp.T / (tmp.sum(axis=1) + 0.001)).T
        tmp[np.arange(len(conf_matrix_with_human)), np.arange(len(conf_matrix_with_human))] = 0
        ...
        mat_to_plot = (1.0 * conf_matrix_with_human.T / (conf_matrix_with_human.sum(axis=1) + 0.001)).T
        ...
        mat_to_plot_sorted = mat_to_plot[idx_with_human_sorted][:, idx_with_human_sorted]
        ...
        def build_model(modelname):
            if modelname == "xgb":
                return __import__("xgboost").XGBClassifier(tree_method="hist", device='cuda', early_stopping_rounds=3)
