...
class AtlasApproxStorer:
    ...
    def store_cell_types(self, comp_data, group):
        ...
        group.create_dataset("obs_names", data=celltypes_tissue.astype("S"))

        # Number of cells
        ncells = comp_data["ncells"]
        group.create_dataset("cell_count", data=ncells.values, dtype="i8")
    ...
    def store_cell_states(self, neid, neigroup):
        ...
        # Number of cells
        ncells = neid["ncells"]
        neigroup.create_dataset("cell_count", data=ncells.values, dtype="i8")
        ...
        # Cell state "names"
        neigroup.create_dataset("obs_names", data=avg.columns.values.astype("S"))
        ...
        neigroup.create_dataset(
            "coords_centroid",
            data=coords_centroids.T.values,
            dtype="f4",
            **self.add_kwargs,
            **self.comp_kwargs,
        )
...
def store_compressed_atlas(
    ...
    if (quantisation == True) or (quantisation == measurement_type):
        if measurement_type == "chromatin_accessibility":
            ...
            qbits = 8
            bins = np.array(
                [-0.001, 1e-8] + np.logspace(-4, 0, 2**qbits - 1).tolist()[:-1] + [1.1]
            )
    ...
    with h5py.File(fn_out, "a") as h5_data:
        ...
        # Store source of data
        me.attrs["source"] = compressed_atlas_with_features["source"]
        ...
        for glabel in glabels:
            ...
            gby_groupgl.create_dataset(
                "dtypes", data=np.array(["object", "object"]).astype("S")
            )
            valgroup = gby_groupgl.create_group("values")
            for gby_level, gby_value, gby_type in zip(
                gby_levels, gby_values, gby_types
            ):
                valgroup.create_dataset(
                    gby_level, data=np.array(gby_value).astype(gby_type)
                )
